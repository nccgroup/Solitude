...
import solitudeCode.blackboxprotobuf.lib.types
from solitudeCode.blackboxprotobuf.lib.types import varint
    ...
    for field_number, value in data.items():
        ...
        if field_type == 'message':
            ...
            if alt_field_number is not None:
                ...
                if field_typedef['message_type_name'] not in solitudeCode.blackboxprotobuf.lib.known_messages:
            ...
            if field_type not in solitudeCode.blackboxprotobuf.lib.types.encoders:
                raise ValueError('Unknown type: %s' % field_type)
            field_encoder = solitudeCode.blackboxprotobuf.lib.types.encoders[field_type]
            if field_encoder is None:
                raise ValueError('Encoder not implemented: %s' % field_type)


        # Encode the tag
        tag = encoder.TagBytes(int(field_number), solitudeCode.blackboxprotobuf.lib.types.wiretypes[field_type])
...
def decode_message(buf, typedef=None, pos=0, end=None, group=False):
    ...
    while pos < end:
        ...
        if field_number in typedef:
            ...
            field_typedef = {}
            field_typedef['type'] = solitudeCode.blackboxprotobuf.lib.types.wire_type_defaults[wire_type]
        ...
        if field_type is None:
            ...
            if field_type == 'message':
                ...
                # Check for type defined by message type name
                    ...
                    message_typedef = solitudeCode.blackboxprotobuf.lib.types.messages[
                        field_typedef['message_type_name']]
                ...
                if solitudeCode.blackboxprotobuf.lib.types.wiretypes[field_type] != wire_type:
                ...
                field_out, pos = solitudeCode.blackboxprotobuf.lib.types.decoders[field_type](buf, pos)
