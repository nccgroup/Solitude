...
import json
import solitudeCode.blackboxprotobuf.lib.types.length_delim
import solitudeCode.blackboxprotobuf.lib.types.type_maps
...
def decode_message(buf, message_type=None):
    ...
    value, typedef, _ = solitudeCode.blackboxprotobuf.lib.types.length_delim.decode_message(buf, message_type)
...
def encode_message(value, message_type):
    ...
    Returns a bytearray
    """
    return solitudeCode.blackboxprotobuf.lib.types.length_delim.encode_message(value, message_type)
...
def validate_typedef(typedef, old_typedef=None):
    ...
    for field_number, field_typedef in typedef.items():
        ...
        for key, value in field_typedef.items():
            ...
            # Validate type value
            if key == "type":
                if value not in solitudeCode.blackboxprotobuf.lib.types.type_maps.wiretypes:
    ...
    if old_typedef is not None:
        ...
        for field_number, value in old_typedef.items():
            wiretype_map[int(field_number)] = solitudeCode.blackboxprotobuf.lib.types.type_maps.wiretypes[value['type']]
        for field_number, value in typedef.items():
            if int(field_number) in wiretype_map:
                old_wiretype = wiretype_map[int(field_number)]
                if old_wiretype != solitudeCode.blackboxprotobuf.lib.types.type_maps.wiretypes[value["type"]]:
